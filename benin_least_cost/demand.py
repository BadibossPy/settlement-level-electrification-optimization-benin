...
def run_demand_model(gdf: pd.DataFrame, config: Optional[ProjectConfig] = None) -> pd.DataFrame:
    ...
    # 2. Vectorized Agricultural Load Estimation
    loads = config.demand.anchor_loads
    agri_demand = np.zeros(len(gdf))
    
    # Rural processing (mills)
    ...
    # Household size (urban: 4.3, rural: 5.2)
    ...
    # Non-residential load estimation
    ...
    # Agri demand already vectorized above
    ...
    # Coincident peak load derivation [kW]
